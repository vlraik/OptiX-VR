...
namespace osc {
  ...
  {
	  PRD &prd = *getPRD<PRD>();
	  const float r = gdt::sqrt(prd.random());
	  const float theta = 2 * prd.random();

	  const float x = r * gdt::cos(theta);
	  const float y = r * gdt::sin(theta);

	  return vec3f(x, y, gdt::sqrt(gdt::max(0.0f, 1 - prd.random())));
  }
