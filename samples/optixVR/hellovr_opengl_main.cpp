...
//-----------------------------------------------------------------------------
// Purpose: Structure to define the Sample Window which is used by optix
//-----------------------------------------------------------------------------
namespace osc {
...
// 		m_SecondsTimer.start(1000, this);
	...
	try {
		...
		osc::Camera camera = { /*from*/osc::vec3f(-1293.07f, 154.681f, -0.7304f),
			/* at */model->bounds.center() - osc::vec3f(0,400,0),
			/* up */osc::vec3f(0.f,1.f,0.f) };

		// some simple, hard-coded light ... obviously, only works for sponza
		const float light_size = 200.f;
		osc::QuadLight light = { /* origin */ osc::vec3f(-1000 - light_size,800,-light_size),
			/* edge 1 */ osc::vec3f(2.f*light_size,0,0),
			/* edge 2 */ osc::vec3f(0,0,2.f*light_size),
			/* power */  osc::vec3f(3000000.f) };

		// something approximating the scale of the world, so the
		// camera knows how much to move for any given user interaction:
		const float worldScale = length(model->bounds.span());

		osc::SampleWindow *window = new osc::SampleWindow("Optix 7 Course Example",
			model, camera, light, worldScale);
		//This is openGL fly mode, not a thing in other example optix codes
